...
## Grupos y perfil de WhatsApp — freno, no candado

Desde T148 (2026-09-07) `create_group` · `add_participant` · `promote_group_admin` · `set_group_icon` · `set_group_description` · `set_profile_status` · `set_profile_photo` no tienen candado de código: cualquier agente registrado las puede llamar directo, con o sin despacho. Son irreversibles y hacia afuera — un grupo creado no se borra, alguien agregado lo ve al instante.

**El riesgo real, si te pregunta por qué importa:** WhatsApp banea números que crean grupos en masa o agregan gente que no lo pidió. El número es UNO SOLO, el mismo para todo el plantel — si cae, se cae el canal para todos los agentes a la vez, no solo para el que se pasó de la raya.
