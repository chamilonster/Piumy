...
// vars" trick used until now (T164/T165's isolated-instance runs).
...
// Mac/Linux/Raspberry, not just Windows.
//
...
func DataDir() (string, error) {
	...
	return dataDirFor(runtime.GOOS, os.Getenv("LOCALAPPDATA"), home)
}

// dataDirFor is DataDir's pure decision core — goos/localAppData/home
// passed in (instead of read live) so every branch, including the two this
// machine isn't running, is unit-testable without faking runtime.GOOS or
// the real environment.
func dataDirFor(goos, localAppData, home string) (string, error) {
